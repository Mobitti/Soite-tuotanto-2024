...
&& isset($_POST['loppu_pvm']))
{
	try
	{
		...
		if($_POST['id'] == "") {
			$sql = "SELECT id FROM kustannus WHERE osasto_id = :osasto_id AND alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm";
			...
			if($row != null) {
				...
				$sql = "INSERT INTO kustannus (id, osasto_id, hinta, alku_pvm, loppu_pvm) VALUES(NULL, :osasto_id, :hinta, :alku_pvm, :loppu_pvm)";
				$values = $con->prepare($sql);
				$values->bindParam(':osasto_id', $_POST['osasto_id']);
			...
			$sql = "SELECT id FROM kustannus WHERE osasto_id = :osasto_id AND id != :id AND alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm";
			$values = $con->prepare($sql);
			$values->bindParam(':osasto_id', $_POST['osasto_id']);
			$values->bindParam(':alku_pvm', $alku_pvm);
			$values->bindParam(':loppu_pvm', $loppu_pvm);
			...
			if($row != null) {
				...
				$sql = "UPDATE kustannus SET osasto_id = :osasto_id, hinta = :hinta, alku_pvm = :alku_pvm, loppu_pvm = :loppu_pvm WHERE id = :id";
				$values = $con->prepare($sql);
				$values->bindParam(':osasto_id', $_POST['osasto_id']);
				$values->bindParam(':hinta', $_POST['hinta']);
				$values->bindParam(':alku_pvm', $alku_pvm);
