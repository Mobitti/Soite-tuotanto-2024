...
&& isset($_POST['kommentti']))
{
	try
	{
		...
		if($_POST['tyojakso_id'] == 0) {
			...
			$sql = "INSERT INTO tyojakso (id, reservilainen_id, osasto_id, alku_pvm, loppu_pvm, tyomaara_id, reservitausta_id, kommentti) 
				VALUES(NULL, :reservilainen_id, :osasto_id, :alku_pvm, :loppu_pvm, :tyomaara_id, :reservitausta_id, :kommentti)";
			$values = $con->prepare($sql);
			$values->bindParam(':reservilainen_id', $_POST['reservilainen_id']);
			...
			for($j = 0; $j < count($aikavalit); $j++)
			{
				...
				if(($j + 1) < count($aikavalit)) {
					...
					if(!($tyojakso_alku_tyyppi == 'L' && $tyojakso_loppu_tyyppi == 'A')) {
						$sql = "SELECT (SELECT prosentti FROM tyomaara WHERE id = tyomaara_id) AS tyomaara_prosentti FROM tyojakso WHERE reservilainen_id = :reservilainen_id AND (alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm) AND id != :id";
						$values = $con->prepare($sql);
						$values->bindParam(':alku_pvm', $tyojakso_alku_pvm);
						$values->bindParam(':loppu_pvm', $tyojakso_loppu_pvm);
						$values->bindParam(':reservilainen_id', $_POST['reservilainen_id']);
			...
			$sql = "UPDATE tyojakso SET osasto_id = :osasto_id, alku_pvm = :alku_pvm, loppu_pvm = :loppu_pvm, 
					tyomaara_id = :tyomaara_id, reservitausta_id = :reservitausta_id, kommentti = :kommentti WHERE id = :id";
			$values = $con->prepare($sql);
			$values->bindParam(':osasto_id', $_POST['osasto_id']);
			$values->bindParam(':alku_pvm', $alku_pvm);
			$values->bindParam(':loppu_pvm', $loppu_pvm);	
