	...
	try
	{
		...
		if($_POST['hakusana'] != "") {
			...
			if(strlen($osasto_idt) > 0) {
				...
				$hakuWhere = " AND osasto_id IN (" . $osasto_idt . ")";
			}
			else {
		...
		switch($_POST['kustannustyyppi']) 
		{
			case 0: $kustannuksetWhere = " AND osasto_id NOT IN (SELECT id FROM osasto WHERE lyhenne LIKE '%Sihteeri%' OR nimi LIKE '%Sihteeri%')"; break;
			case 1: $kustannuksetWhere = " AND osasto_id IN (SELECT id FROM osasto WHERE lyhenne LIKE '%Sihteeri%' OR nimi LIKE '%Sihteeri%')"; break;
		}
		
		$jarjestysOrder = " ORDER BY " . $_POST['jarjestettavaarvo'] . " " . $_POST['jarjestys'];
		
		$sql = "SELECT id, osasto_id,(SELECT raporttinumero FROm osasto WHERE id = osasto_id) AS raporttinumero, (SELECT nimi FROM osasto WHERE id = osasto_id) AS osasto, hinta, alku_pvm, loppu_pvm FROM kustannus WHERE id != 0" . $hakuWhere . $kustannuksetWhere . $jarjestysOrder;
		$values = $con->prepare($sql);
		$values->execute();
