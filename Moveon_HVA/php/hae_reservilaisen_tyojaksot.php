...
if(isset($_POST['reservilainen_id'])
	...
	try
	{
		...
		if($_POST['hakusana'] != "") {
			switch($haettavaarvo) 
			{
				...
				case 'reservitausta': $hakuWhere = " AND reservitausta_id IN(SELECT id FROM reservitausta WHERE selite LIKE '%" . $_POST['hakusana'] . "%' OR numero = '" . $_POST['hakusana'] . "')"; break; 
		...
		{		
			...
			case 'reservitausta': $jarjestysarvo = "(SELECT numero FROM reservitausta WHERE id = reservitausta_id)"; break;
		...
		$sql = "SELECT id, reservilainen_id, osasto_id, alku_pvm, loppu_pvm, tyomaara_id, reservitausta_id, kommentti FROM tyojakso WHERE reservilainen_id = :reservilainen_id" . $pvmWhere . $hakuWhere . " ORDER BY " . $jarjestysarvo . " " . $_POST['jarjestys'];
		$values = $con->prepare($sql);
		$values->bindParam(':reservilainen_id', $_POST['reservilainen_id']);
		$values->execute();
		while($row = $values->fetch(PDO::FETCH_ASSOC)) 
