...
if (
	...
	try {
		...
		if ($_POST['hakusana'] != "") {
			switch ($haettavaarvo) {
				case 'osasto':
					$hakuWhere = " AND osasto_id IN(SELECT id FROM osasto WHERE nimi LIKE '%" . $_POST['hakusana'] . "%' OR lyhenne LIKE '%" . $_POST['hakusana'] . "%')";
					break;
				case 'prosentti':
					$hakuWhere = " AND tyomaara LIKE '%" . $_POST['hakusana'] . "%'";
					break;
				case 'reservitausta':
					$hakuWhere = " AND reservitausta_id IN(SELECT id FROM reservitausta WHERE selite LIKE '%" . $_POST['hakusana'] . "%' OR numero = '" . $_POST['hakusana'] . "')";
					break;
				case 'kommentti':
					$hakuWhere = " AND kommentti LIKE '%" . $_POST['hakusana'] . "%'";
					break;
			}
		}
		...
		switch ($_POST['jarjestettavaarvo']) {
			case 'alku_pvm':
				$jarjestysarvo = "alku_pvm";
				break;
			case 'loppu_pvm':
				$jarjestysarvo = "loppu_pvm";
				break;
			case 'osasto':
				$jarjestysarvo = "(SELECT lyhenne FROM osasto WHERE id = osasto_id)";
				break;
			case 'prosentti':
				$jarjestysarvo = "tyomaara";
				break;
			case 'reservitausta':
				$jarjestysarvo = "(SELECT numero FROM reservitausta WHERE id = reservitausta_id)";
				break;
			case 'kommentti':
				$jarjestysarvo = "kommentti";
				break;
		}
		...
		if ($_POST['varoitus'] == 0) {
			...
			$sql = "SELECT (CASE WHEN (:alku_pvm > alku_pvm) THEN :alku_pvm ELSE alku_pvm END) AS pvm, 'A' AS tyyppi FROM tyojakso WHERE reservilainen_id = :reservilainen_id" . $pvmWhere . " UNION SELECT (CASE WHEN (:loppu_pvm < loppu_pvm) THEN :loppu_pvm ELSE loppu_pvm END) AS pvm, 'L' AS tyyppi FROM tyojakso WHERE reservilainen_id = :reservilainen_id" . $pvmWhere . " ORDER BY pvm ASC";
		...
		$sql = "SELECT COUNT(id) AS tyojaksot_yhteensa FROM tyojakso WHERE reservilainen_id = :reservilainen_id" . $pvmWhere;
		$values = $con->prepare($sql);
		$values->bindParam(':reservilainen_id', $_POST['reservilainen_id']);
		$values->execute();
		$row = $values->fetch(PDO::FETCH_ASSOC);
		...
		//Haetaan näytettävät työjakso tiedot
		$sql = "SELECT id, reservilainen_id, osasto_id, alku_pvm, loppu_pvm, tyomaara, reservitausta_id, kommentti FROM tyojakso WHERE reservilainen_id = :reservilainen_id" . $pvmWhere . $hakuWhere . " ORDER BY " . $jarjestysarvo . " " . $_POST['jarjestys'];
		$values = $con->prepare($sql);
		$values->bindParam(':reservilainen_id', $_POST['reservilainen_id']);
		$values->execute();
		while ($row = $values->fetch(PDO::FETCH_ASSOC)) {
			...
			$row_array['tyomaara'] = $row['tyomaara'];
			$row_array['reservitausta_id'] = $row['reservitausta_id'];
