...
&& isset($_POST['loppu_pvm']))
{
	try
	{
		...
		if($_POST['id'] == "") {
			$sql = "SELECT id FROM kmkustannus WHERE alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm AND tyyppi = :tyyppi";
			...
			if($row != null) {
				...
				$sql = "INSERT INTO kmkustannus (id, tyyppi, hinta, alku_pvm, loppu_pvm) VALUES(NULL, :tyyppi, :hinta, :alku_pvm, :loppu_pvm)";
				$values = $con->prepare($sql);
				$values->bindParam(':tyyppi', $_POST['tyyppi']);
			...
			$sql = "SELECT id FROM kmkustannus WHERE id != :id AND alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm AND tyyppi = :tyyppi";
			$values = $con->prepare($sql);
			$values->bindParam(':alku_pvm', $alku_pvm);
			$values->bindParam(':loppu_pvm', $loppu_pvm);
			$values->bindParam(':tyyppi', $_POST['tyyppi']);
			...
			if($row != null) {
				...
				$sql = "UPDATE kmkustannus SET tyyppi = :tyyppi, hinta = :hinta, alku_pvm = :alku_pvm, loppu_pvm = :loppu_pvm WHERE id = :id";
				$values = $con->prepare($sql);
				$values->bindParam(':tyyppi', $_POST['tyyppi']);
				$values->bindParam(':hinta', $_POST['hinta']);
				$values->bindParam(':alku_pvm', $alku_pvm);
