...
&& isset($_POST['loppu_pvm']))
{
	try
	{
		...
		if($_POST['id'] == "") {
			$sql = "SELECT id FROM sihteerikustannus WHERE alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm";
			...
			if($row != null) {
				...
				$sql = "INSERT INTO sihteerikustannus (id, hinta, alku_pvm, loppu_pvm) VALUES(NULL, :hinta, :alku_pvm, :loppu_pvm)";
				$values = $con->prepare($sql);
			...
			$sql = "SELECT id FROM sihteerikustannus WHERE id != :id AND alku_pvm <= :loppu_pvm AND loppu_pvm >= :alku_pvm";	
			$values = $con->prepare($sql);
			$values->bindParam(':alku_pvm', $alku_pvm);
			$values->bindParam(':loppu_pvm', $loppu_pvm);
			...
			if($row != null) {
				...
				$sql =  "UPDATE sihteerikustannus SET hinta = :hinta, alku_pvm = :alku_pvm, loppu_pvm = :loppu_pvm WHERE id = :id";
				$values = $con->prepare($sql);
				$values->bindParam(':hinta', $_POST['hinta']);
				$values->bindParam(':alku_pvm', $alku_pvm);
				$values->bindParam(':loppu_pvm', $loppu_pvm);
